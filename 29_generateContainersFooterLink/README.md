...
Learn how to use container's own props inside the `mapStateToProps()` and `mapDispatchToProps()` functions from [the video by Dan Abramov](https://egghead.io/lessons/javascript-redux-generating-containers-with-connect-from-react-redux-footerlink).
